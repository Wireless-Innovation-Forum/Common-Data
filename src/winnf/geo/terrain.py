...
Typical usage:
  ...
  # Initialize the terrain driver
  driver = TerrainDriver(cache_size=16)
...
class TerrainDriver:
  ...
  def SetTerrainDirectory(self, terrain_directory):
    ...
    if self._terrain_dir is None:
      self._terrain_dir = winnf.GetNedDir()
...
def TerrainProfile(driver, lat1, lon1, lat2, lon2,
                   target_res_meter=-1,
                   target_res_arcsec=1,
                   do_interp=True,
                   max_points=-1):
  ...
  elev.extend(driver.GetTerrainElevation(lats, lons, do_interp))
  return elev


def ComputeNormalizedHaat(driver, lat, lon):
  """Computes normalized HAAT (Height Above Average Terrain).

  Args:
    lat, lon: The point coordinates (in degrees).

  Returns:
    A tuple (haat, altitude) of:
      - the HAAT for an antenna at height 0 above ground level
      - the terrain altitude at the given location
  """
  ...
  altitudes = driver.GetTerrainElevation(all_lat, all_lon)
