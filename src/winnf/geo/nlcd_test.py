...
class TestNlcd(unittest.TestCase):
  ...
  @classmethod
  def setUpClass(cls):
    cls.unzip_files = UnzipTestDir(TEST_DIR)
  ...
  def tearDownClass(cls):
    RemoveFiles(TEST_DIR, cls.unzip_files)
  ...
  def test_region_vote(self):
    points = [(37.751113, -122.449722),  # Urban
              ...
              (37.750036, -122.51527)]     # Rural
    self.assertEqual(nlcd.RegionNlcdVote(self.nlcd_driver, points[0:1]), 'URBAN')
    self.assertEqual(nlcd.RegionNlcdVote(self.nlcd_driver, points[0:2]), 'URBAN')
    self.assertEqual(nlcd.RegionNlcdVote(self.nlcd_driver, points[0:3]), 'SUBURBAN')
    self.assertEqual(nlcd.RegionNlcdVote(self.nlcd_driver, points[0:4]), 'SUBURBAN')
    self.assertEqual(nlcd.RegionNlcdVote(self.nlcd_driver, points[0:5]), 'RURAL')
  ...
  def test_vote_undefd(self):
    points = [(37.75, -124.2)]
    reg_type = nlcd.RegionNlcdVote(self.nlcd_driver, points, out_forbid=False)
    self.assertEqual(reg_type, 'RURAL')
    with self.assertRaises(ValueError):
      reg_type = nlcd.RegionNlcdVote(self.nlcd_driver, points, out_forbid=True)
