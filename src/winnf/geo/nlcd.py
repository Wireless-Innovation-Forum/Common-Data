...
class NlcdDriver:
  ...
  def SetNlcdDirectory(self, nlcd_directory):
    ...
    if self._nlcd_dir is None:
      self._nlcd_dir = winnf.GetNlcdDir()
...
def RegionNlcdVote(driver, points, out_forbid=False):
  """Vote on most common NLCD in a region.

  According to WinnForum spec R2-SGN-04.

  Inputs:
    driver: A NLCD driver.
    points: A list of (lat, lon) tuples defining a region
  ...
  codes = driver.GetLandCoverCodes(lat, lon)
