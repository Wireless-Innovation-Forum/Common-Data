...
download_dir = os.path.join(cur_dir, '..', 'data', 'orig_ned2')

# Output dir for final NED tiles
out_dir =  os.path.join(cur_dir, '..', 'data', 'ned_out2')

# Reference dir for existing 1'' NED tiles
ref_dir =  os.path.join(cur_dir, '..', 'data', 'ned')

print('Retrieving USGS 2-arc-second tiles to dir=%s' % download_dir)
...
if not os.path.exists(ref_dir):
  raise Exception('reference dir %s does not exist' % ref_dir)

os.chdir(download_dir)

# Retrieve all the newest tiles into destination dir and create the tiles in the
# reference directory
RetrieveMissingElevationTilesAndProcess(out_dir, ref_dir)
