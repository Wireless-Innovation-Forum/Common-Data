...
dest_dir = os.path.join(rootDir, 'data', 'census')
print('Converting shapefile files in %s' % dest_dir)

files = os.listdir(dest_dir)
print('Found %d zip files to translate' % len(files))
for f in files:
  if os.path.isfile(dest_dir + '/' + f) and re.match('.*\.zip$', f):
    TranslateShpFileToKML(dest_dir + '/' + f)
